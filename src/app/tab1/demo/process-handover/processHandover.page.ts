...
@Component({
    selector: 'app-process-handover',
    templateUrl: './processHandover.page.html',
    styleUrls: ['./processHandover.page.scss'],
})
export class ProcessHandoverPage implements OnInit, AfterViewInit {
    // @ts-ignore
    ...
    params = {
        ...
        opnum: '',    //    工序号
        opname: '',   //    工序名称
        sourcetype: 'not4'  // 来源类型
    };

    ngOnInit() {
        if (!this.searchValue) {
            this.getProcessHandoverList();
        } else {
            this.search();
        }
    }
    ...
    /**
     * getProcessHandoverList()
     * 获取 工业交接 列表数据
     */
    getProcessHandoverList() {
        this.processReportService.allProcessHandover(this.params).then(res => {
            if (res['rows']) {
                this.handoverList = res['rows'];
                this.records = res['records'];
                this.spliceViewList(this.handoverList);
            } else if (res === false) {
                console.log('获取工序交接列表数据数据量为0');
                this.noData = true;
            }
        }, err => {
    ...
    search() {
        ...
        this.processReportService.allProcessHandover(this.params).then(res => {
            ...
            this.records = res['records'];
            this.spliceViewList(this.handoverList);
            console.log('search-reportList = ', this.handoverList);
            ToastService.hide();
        }, err => {
        });
    }
    ...
    async doInfinite(infiniteScroll) {
        if (this.records > this.handoverList.length) {
            this.loadingMoreText = '上拉加载更多数据...';
            // console.log('开始异步操作');
            // this.page = this.page + 1;          // 请求的页数 第几页
            // this.params = `?rows=${this.rows}&&page=${this.page}&&sort=${this.sort}`;
            this.params.page += 1;
            await this.processReportService.allProcessHandover(this.params).then(res => {
                this.handoverList = this.handoverList.concat(res['rows']);
                this.records = res['records'];
                // 显示分页数据
                this.spliceViewList(this.handoverList);
