...
function importJson(spreadJson) {
    ...
    if (spreadJson.version && spreadJson.sheets) {
        ...
        syncSpreadPropertyValues();
        syncSheetPropertyValues();
        // bbb 获取单元格焦点位置并将数据赋值给单元格
        var sheet = spread.getActiveSheet();
        // formsheet变量 应用在common.js
        formsheet = sheet;
        spreadss = spread;
      ...
      // }
        ...
        //     		if(item.id && item.level==1){
        //                 sheet.getCell(args.row, args.col).value("["+item.id+"]");
        //                 // 字段设置数据库存取
        //                 var dataText = "formsheet.getCell("+args.row+"," +args.col+").bindingPath('"+item.id+"')";
        //                 $dataServiceProxy.invokeMethod('Genersoft.SCM.PSKB.Core.PSKBJsReport','SaveMouldField',[mouldID,item.id,dataText]).then(function(result){

        //                 });
        //     		}else if(item.id && item.level==2){
        //     			var pNode = item.getParentNode();
        //     			sheet.getCell(args.row, args.col).value("["+pNode.name+"."+item.id+"]");
        //     		}
        //     	});
        //     	treeDOM1.cancelSelectedNode();
        // 	}
        // });
